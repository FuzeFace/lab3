...
В процессе выполнения лабораторной работы возникло несколько технических особенностей.

Во-первых, при первом запуске Whisper язык видео был указан как русский, хотя видео было на английском языке. Из-за этого транскрипт получился некорректным. После изменения параметра языка на `en` транскрипция стала корректной.

Во-вторых, PySceneDetect не обнаружил явных смен сцен. Это связано с тем, что видео визуально статичное: спикер находится у доски, а смена визуального содержимого происходит постепенно. Поэтому было принято решение использовать равномерное разбиение видео на 8 временных фрагментов.

В-третьих, OpenCV не смог корректно извлечь кадры из видео в Google Colab. Для решения этой проблемы был использован `ffmpeg`, после чего ключевые кадры успешно сохранились.

В-четвёртых, в текущей версии библиотеки `transformers` задача `pipeline("image-to-text")` не поддерживалась. Поэтому BLIP был загружен напрямую через `BlipProcessor` и `BlipForConditionalGeneration`.
